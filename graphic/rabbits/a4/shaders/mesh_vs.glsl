...
void main() {
   ...
   //vec4 newPosition = vec4(a_Position.x, a_Position.y, a_Position.z, 1.0);
    ...
    float yoffset = 0.0;
    float zoffset = 0.0;
    if (a_Position.y>0.133) {
        yoffset = u_DistortionAmp*0.2*(1.0+sin(3.0*u_DistortionTime));
    }
    if (a_Position.y<0.055) {
        yoffset = u_DistortionAmp*0.5*(1.0+cos(3.0*u_DistortionTime));
    }
    if(a_Position.z>0.1){
        zoffset = u_DistortionAmp*0.5*(1.0+cos(3.0*u_DistortionTime));
    }
    
    vec4 newPosition = vec4(a_Position.x, a_Position.y + yoffset, a_Position.z+zoffset, 1.0);
